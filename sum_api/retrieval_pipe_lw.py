...
def worker(data, sum_type, openai_key, q):
...
def get_final_result(message, max_token, openai_key, history_sum=''):
    ...
    processes = []
    for dl in data_ls:
        p = multiprocessing.Process(target=worker, args=(dl, 'conv2sum', openai_key, q))
        processes.append(p)
        p.start()
    ...
    while len(res)!=1:
        ...
        processes = []
        for sl in sum_ls:
            p = multiprocessing.Process(target=worker, args=(sl, 'sum2sum', openai_key, q))
            processes.append(p)
            p.start()
